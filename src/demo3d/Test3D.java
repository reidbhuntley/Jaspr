...
public class Test3D {
	...
	public static void main(String[] args) throws InterruptedException, ExecutionException {
		...
		GameWindow window = GameWindow.create(WINDOW_WIDTH, WINDOW_HEIGHT, new Renderer(es, FOV, NEAR_FRAME, FAR_FRAME));
		
		meshes = window.getMeshManager();
		meshes.preload();
		textures = window.getTextureManager();
		textures.preload();
		...
		for (int i = 0; i < 70; i++) {
			for (int j = 0; j < 70; j++) {
				new Entity(meshes.get("capricorn.obj"), new Position3((i - 35) * 60, -50, (j - 35) * 60, 0, 0, 0));
			}
		}
